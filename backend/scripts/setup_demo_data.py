...
def build_product_rows(start_index: int, count: int) -> list[dict]:
        ...
        rows.append(
            {
                ...
                "category": category,
                "description": f"{description}. Color {color}. Talla {size}. Linea demo #{index + 1}.",
                "price": price,
                "cost": cost,
                "stock": stock,
                "reserved": 0,
...
async def main() -> None:
    async with AsyncSessionLocal() as db:
        total_products = (await db.execute(text("SELECT COUNT(*) FROM products"))).scalar_one()
        target_total = 1000
        missing = max(0, target_total - total_products)
        ...
        if missing:
            await db.execute(
                text(
                    ...
                    (name, category, description, price, cost, stock, reserved, margin_pct, sku, image_url, is_active, ai_priority, created_at, updated_at)
                    VALUES
                    (:name, :category, :description, :price, :cost, :stock, :reserved, :margin_pct, :sku, :image_url, :is_active, :ai_priority, :created_at, :updated_at)
                    """
                ),
                build_product_rows(total_products, missing),
            )
        ...
        closed_conversations = (
            await db.execute(
                text(
                    ...
                    SET status = 'CLOSED', ai_active = false, closed_at = COALESCE(closed_at, :now), updated_at = :now
                    """
                ),
                {"now": now},
            )
        ...
        for idx, name in enumerate(CUSTOMER_NAMES, start=1):
            result = await db.execute(
                text(
                    ...
                    (name, phone, email, channel, is_blocked, total_spent, created_at)
                    VALUES
                    (:name, :phone, :email, 'WHATSAPP', false, 0, :created_at)
                    RETURNING id
                    """
                ),
                {
        ...
        for customer_id in customer_ids:
            await db.execute(
                text(
                    ...
                    (customer_id, channel, status, ai_active, created_at, updated_at, closed_at)
                    VALUES
                    (:customer_id, 'WHATSAPP', 'OPEN', true, :created_at, :updated_at, NULL)
                    """
                ),
                {
        ...
        await db.commit()
        print(
            f"Demo data ready: total_products={total_products + missing}, "
            f"closed_conversations={closed_conversations}, new_conversations={len(customer_ids)}"
        )
